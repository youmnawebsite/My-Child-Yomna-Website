...
const games = [
  { id: 'memory', name: "شوفي الصورتين المتشابهين", icon: "🧠", color: "bg-pink-200" },
  ...
  { id: 'numbers', name: "شغلي دماغك يغبيه ", icon: "🔢", color: "bg-purple-200" },
...
export default function Home() {
  return (
    <div className="min-h-screen bg-gradient-to-r from-pink-100 to-blue-100 p-4 font-cairo">
      ...
      <p className="text-2xl text-center mb-8 text-purple-600 animate-pulse">كل الألعاب دي عشان بنوتي تبقى مبسوطه كل ما تزهقي يبنوتي خشي هنا والعبي براحتك❤️</p>
          ...
          <Link href={`/games/${game.id}`} key={game.id}>
            <Card className={`${game.color} hover:shadow-xl transition-all duration-300 transform hover:scale-105 cursor-pointer rounded-2xl overflow-hidden`}>
              <CardHeader className="bg-white bg-opacity-30">
                <CardTitle className="text-xl sm:text-2xl font-bold text-center text-purple-700">{game.name}</CardTitle>
              </CardHeader>
              <CardContent>
